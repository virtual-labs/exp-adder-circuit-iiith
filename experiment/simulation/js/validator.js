...
function fullAdderTest(input0,input1,carryIn,carryOut,sumOut)
{
    ...
    for(var i=0; i<8;i++)
    {
        ...
        var tableRow = '<tr><th>1</th><td>'+ input0.output +'</td><td>More stuff</td><td>Lorem ipsum</td><td>Lorem ipsum dolor sit amet consectetur adipisicing elit.Laborum, ut.</td></tr>'
...
function rippleAdderTest(inputA0,inputB0,inputA1,inputB1,inputA2,inputB2,inputA3,inputB3,inputCin,OutputCout,OutputS0,OutputS1,OutputS2,OutputS3)
{
    ...
    for(var i=0;i<512;i++) // 512 = 2^9 basically calculates all the possible combinations for 9 inputs
    {
        ...
        var sumS1 = (aXorb2 && !carryC1.output) || (!aXorb2 && carryC1.output);
        var carryC2 = (inputA1.output && inputB1.output) || (aXorb2 && carryC1.output)


        // FOR THIRD ADDER
        var aXorb3 =  (inputA2.output && !inputB2.output) || (!inputA2.output && inputB2.output);
        var sumS2 = (aXorb3 && !carryC2.output) || (!aXorb3 && carryC2.output);
        var carryC3 = (inputA2.output && inputB2.output) || (aXorb3 && carryC2.output)


        // FOR FOURTH ADDER
        var aXorb4 =  (inputA3.output && !inputB3.output) || (!inputA3.output && inputB3.output);
        var sumS3 = (aXorb4 && !carryC3.output) || (!aXorb4 && carryC3.output);
        var carryCout = (inputA3.output && inputB3.output) || (aXorb4 && carryC3.output)
        
        // simulate the circuit
        testSimulationFA(FA,Gates)
        var sumSout0 = Gates[OutputS0].sum;
        var sumSout1 = Gates[OutputS1].sum;
        var sumSout2 = Gates[OutputS2].sum;
        var sumSout3 = Gates[OutputS3].sum;
        var carryOut = Gates[OutputCout].Cout;
